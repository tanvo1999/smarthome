...
function validateAndroidFullScreenAction(fullScreenAction) {
    if (!utils_1.isObject(fullScreenAction)) {
        throw new Error("'fullScreenAction' expected an object value.");
    }
    if (!utils_1.isString(fullScreenAction.id) || fullScreenAction.id.length === 0) {
        throw new Error("'id' expected a non-empty string value.");
    }
    const out = {
        id: fullScreenAction.id,
    };
    if (!utils_1.isUndefined(fullScreenAction.launchActivity)) {
        if (!utils_1.isString(fullScreenAction.launchActivity)) {
    ...
    if (!utils_1.isUndefined(fullScreenAction.launchActivityFlags)) {
        if (!utils_1.isArray(fullScreenAction.launchActivityFlags)) {
        ...
        if (fullScreenAction.launchActivityFlags.length) {
            if (!utils_1.isNumber(fullScreenAction.launchActivityFlags[0])) {
                throw new Error("'launchActivityFlags' must be an array of `AndroidLaunchActivityFlag` values.");
            }
        }
        out.launchActivityFlags = fullScreenAction.launchActivityFlags;
    }
    if (!utils_1.isUndefined(fullScreenAction.mainComponent)) {
        if (!utils_1.isString(fullScreenAction.mainComponent)) {
